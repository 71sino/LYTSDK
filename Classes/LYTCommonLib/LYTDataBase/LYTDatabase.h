...
NS_ASSUME_NONNULL_BEGIN
    ...
    #define LYTDBAutorelease(__v) ([__v autorelease]);
    #define LYTDBReturnAutoreleased LYTDBAutorelease

    #define LYTDBRetain(__v) ([__v retain]);
    #define LYTDBReturnRetained LYTDBRetain

    #define LYTDBRelease(__v) ([__v release]);

    #define LYTDBDispatchQueueRelease(__v) (dispatch_release(__v));
#else
    // -fobjc-arc
    #define LYTDBAutorelease(__v)
    #define LYTDBReturnAutoreleased(__v) (__v)

    #define LYTDBRetain(__v)
    #define LYTDBReturnRetained(__v) (__v)

    #define LYTDBRelease(__v)

// If OS_OBJECT_USE_OBJC=1, then the dispatch objects will be treated like ObjC objects
// and will participate in ARC.
// See the section on "Dispatch Queues and Automatic Reference Counting" in "Grand Central Dispatch (GCD) Reference" for details. 
    #if OS_OBJECT_USE_OBJC
        #define LYTDBDispatchQueueRelease(__v)
    #else
        #define LYTDBDispatchQueueRelease(__v) (dispatch_release(__v));
    #endif
#endif
...
typedef int(^LYTDBExecuteStatementsCallbackBlock)(NSDictionary *resultsDictionary);


/** A SQLite ([http://sqlite.org/](http://sqlite.org/)) Objective-C wrapper.
 
 ### Usage
 The three main classes in LYTDB are:

 - `LYTDatabase` - Represents a single SQLite database.  Used for executing SQL statements.
 - `<LYTResultSet>` - Represents the results of executing a query on an `LYTDatabase`.
 - `<LYTDatabaseQueue>` - If you want to perform queries and updates on multiple threads, you'll want to use this class.

 ### See also
 
 - `<LYTDatabasePool>` - A pool of `LYTDatabase` objects.
 - `<LYTStatement>` - A wrapper for `sqlite_stmt`.
 
 ### External links
 
 - [LYTDB on GitHub](https://github.com/ccgus/LYTdb) including introductory documentation
 - [SQLite web site](http://sqlite.org/)
 - [LYTDB mailing list](http://groups.google.com/group/LYTdb)
 - [SQLite FAQ](http://www.sqlite.org/faq.html)
 
 @warning Do not instantiate a single `LYTDatabase` object and use it across multiple threads. Instead, use `<LYTDatabaseQueue>`.
...
@interface LYTDatabase : NSObject
...
/** Create a `LYTDatabase` object.
 
 An `LYTDatabase` is created with a path to a SQLite database file.  This path can be one of these three:

 1. A file system path.  The file does not have to exist on disk.  If it does not exist, it is created for you.
 2. An empty string (`@""`).  An empty database is created at a temporary location.  This database is deleted with the `LYTDatabase` connection is closed.
 3. `nil`.  An in-memory database is created.  This database will be destroyed with the `LYTDatabase` connection is closed.

 For example, to create/open a database in your Mac OS X `tmp` folder:

    LYTDatabase *db = [LYTDatabase databaseWithPath:@"/tmp/tmp.db"];

 Or, in iOS, you might open a database in the app's `Documents` directory:

    NSString *docsPath = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES)[0];
    NSString *dbPath   = [docsPath stringByAppendingPathComponent:@"test.db"];
    LYTDatabase *db     = [LYTDatabase databaseWithPath:dbPath];
 ...
 @return `LYTDatabase` object if successful; `nil` if failure.
...
/** Create a `LYTDatabase` object.
 ...
 2. `nil`.  An in-memory database is created.  This database will be destroyed with the `LYTDatabase` connection is closed.
...
/** Initialize a `LYTDatabase` object.
...
/** Initialize a `LYTDatabase` object.
 ...
 An `LYTDatabase` is created with a local file URL to a SQLite database file.  This path can be one of these three:
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(__attribute__((noescape)) LYTDBExecuteStatementsCallbackBlock _Nullable)block;
...
/** Execute select statement
 ...
 Executing queries returns an `<LYTResultSet>` object if successful, and `nil` upon failure.  Like executing updates, there is a variant that accepts an `NSError **` parameter.  Otherwise you should use the `<lastErrorMessage>` and `<lastErrorMessage>` methods to determine why a query failed.
 
 In order to iterate through the results of your query, you use a `while()` loop.  You also need to "step" (via `<[LYTResultSet next]>`) from one record to the other.
 ...
 @return A `<LYTResultSet>` for the result set upon success; `nil` upon failure. If failed, you can call `<lastError>`, `<lastErrorCode>`, or `<lastErrorMessage>` for diagnostic information regarding the failure.
 
 @see LYTResultSet
 @see [`LYTResultSet next`](<[LYTResultSet next]>)
 @see [`sqlite3_bind`](http://sqlite.org/c3ref/bind_blob.html)
...
- (LYTResultSet * _Nullable)executeQuery:(NSString*)sql, ...;
...
- (LYTResultSet * _Nullable)executeQueryWithFormat:(NSString*)format, ... NS_FORMAT_FUNCTION(1,2);
...
- (LYTResultSet * _Nullable)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments;
...
/** Execute select statement
 ...
 `func executeQuery(sql: String, values: [Any]?) throws  -> LYTResultSet!`
...
- (LYTResultSet * _Nullable)executeQuery:(NSString *)sql values:(NSArray * _Nullable)values error:(NSError * _Nullable __autoreleasing *)error;
...
- (LYTResultSet * _Nullable)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary * _Nullable)arguments;


// Documentation forthcoming.
- (LYTResultSet * _Nullable)executeQuery:(NSString *)sql withVAList:(va_list)args;
...
/** Adds SQL functions or aggregates or to redefine the behavior of existing SQL functions or aggregates.
 ...
 For example:
    ...
    LYTResultSet *rs = [db executeQuery:@"SELECT * FROM employees WHERE RemoveDiacritics(first_name) LIKE 'jose'"];
...
/** Generate an `NSDateFormatter` that won't be broken by permutations of timezones or locales.
 ...
 Example:
    ...
    myDB.dateFormat = [LYTDatabase storeableDateFormat:@"yyyy-MM-dd HH:mm:ss"];
 ...
 @warning Note that `NSDateFormatter` is not thread-safe, so the formatter generated by this method should be assigned to only one LYTDB instance and should not be used for other purposes.
...
/** Set to a date formatter to use string dates with sqlite instead of the default UNIX timestamps.
 ...
 @param format Set to nil to use UNIX timestamps. Defaults to nil. Should be set using a formatter generated using LYTDatabase::storeableDateFormat.
 ...
 @warning Note there is no direct getter for the `NSDateFormatter`, and you should not use the formatter you pass to LYTDB for other purposes, as `NSDateFormatter` is not thread-safe.
 */
...
/** Objective-C wrapper for `sqlite3_stmt`
 ...
 This is a wrapper for a SQLite `sqlite3_stmt`. Generally when using LYTDB you will not need to interact directly with `LYTStatement`, but rather with `<LYTDatabase>` and `<LYTResultSet>` only.
...
@interface LYTStatement : NSObject {
    void *_statement;
    NSString *_query;
    long _useCount;
    BOOL _inUse;
