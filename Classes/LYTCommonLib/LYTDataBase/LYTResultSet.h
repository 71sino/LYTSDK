...
/** Represents the results of executing a query on an `<LYTDatabase>`.
...
@interface LYTResultSet : NSObject

@property (nonatomic, retain, nullable) LYTDatabase *parentDB;
...
/** `LYTStatement` used by result set. */

@property (atomic, retain, nullable) LYTStatement *statement;

///------------------------------------
/// @name Creating and closing database
///------------------------------------

/** Create result set from `<LYTStatement>`
 
 @param statement A `<LYTStatement>` to be performed
 
 @param aDB A `<LYTDatabase>` to be used
 
 @return A `LYTResultSet` on success; `nil` on failure
 */

+ (instancetype)resultSetWithStatement:(LYTStatement *)statement usingParentDatabase:(LYTDatabase*)aDB;
